...
样例代码1中，`fn2()`就是一个闭包，他访问到了fn1()中的变量,成功输出了`iceman`。
...
在样例代码3中，`f1`执行后，`f3`被赋值，此时`f3`拥有了**f3、f1、全局作用域的访问权限**。逐层向上查找，最后输出2。
...
样例代码4中的变量查询过程都是在**创建作用域而不是调用作用域**中查找。这体现了作用域链以及闭包的性质。
...
**解析：**
`setTimeout`是异步函数。在执行该函数时，外部循环已经执行完毕。由于`for`不会创作用域，所以使用`var`定义的变量`i`实际上是在全局作用域下的，因此只有一个`i`。在执行`setTimeout`时，向上寻找变量`i`，在全局作用域下找到`i = 6`，因此全部输出6。

解决方法：

- 使用闭包。当每次循环时，将此时的变量i作为参数传入,这样作用域便被保存下来，可以正确输出。
